#ifndef ANDROID_HARDWARE_NEURALNETWORKS_V1_1_GLES_CPU_TIMER_H
#define ANDROID_HARDWARE_NEURALNETWORKS_V1_1_GLES_CPU_TIMER_H
...
{
public:
    GlesCpuTimer(const std::string& n) : start(0), name(n), opTimer(nullptr) { init(); }
    GlesCpuTimer(OperationCpuTimer* t) : start(0), opTimer(t) { init(); }
    ~GlesCpuTimer() { deinit(); }
private:
    GlesCpuTimer();
    void init()
